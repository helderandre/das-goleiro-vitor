...
import { resendOrderEmail } from "@/app/(dashboard)/pedidos/actions"
...
export function OrderEmails({ orderId, emails, events }: OrderEmailsProps) {
  if (emails.length === 0) {
    return (
  ...
  for (const ev of events) {
    eventsByEmail.set(ev.outbox_id, [...(eventsByEmail.get(ev.outbox_id) ?? []), ev])
  }
...
function EmailRow({
  ...
  const status = STATUS[email.status] ?? { label: email.status, variant: "outline" as const }
  const delivery = email.delivery_status ? DELIVERY[email.delivery_status] : null
  // Motivo da rejeição vem no evento, não na linha da fila.
  const rejection = events
    .filter((e) => (e.type === "bounced" || e.type === "complained") && e.detail)
    .at(-1)?.detail
  const reason = email.status === "failed" || email.status === "skipped" ? email.last_error : null
  const retrying = email.status === "pending" && email.attempts > 0 ? email.last_error : null
    ...
    <div className={isResend ? "border-l-2 pl-3" : undefined}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-medium">
          ...
          {isResend && (
            <span className="ml-2 text-xs font-normal text-muted-foreground">reenvio</span>
          )}
        </p>
        <div className="flex flex-wrap gap-1.5">
          {email.status !== "sent" && <Badge variant={status.variant}>{status.label}</Badge>}
      ...
      <p className="mt-0.5 break-all text-xs text-muted-foreground">
        {email.recipient ?? "—"} · {formatDateTime(email.sent_at ?? email.created_at)}
      </p>
      ...
      {rejection && <p className="mt-1 text-xs text-destructive">{rejection}</p>}
      ...
      {events.length > 0 && (
        <details className="group mt-2">
          <summary className="cursor-pointer list-none text-xs text-muted-foreground hover:text-foreground [&::-webkit-details-marker]:hidden">
            <span className="group-open:hidden">Ver linha do tempo ({events.length})</span>
            <span className="hidden group-open:inline">Ocultar linha do tempo</span>
          </summary>
          <ol className="mt-2 space-y-1 border-l pl-3">
            {events.map((ev) => (
              <li key={ev.id} className="text-xs">
                <span className="font-medium">{DELIVERY[ev.type]?.label ?? ev.type}</span>
                <span className="text-muted-foreground"> · {formatDateTime(ev.occurred_at)}</span>
                {ev.detail && (
                  <span className="block break-all text-muted-foreground">{ev.detail}</span>
                )}
              </li>
            ))}
          </ol>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {DELIVERY[email.delivery_status ?? ""]?.label.toLowerCase() ?? "entregue"} para{" "}
              {email.recipient}. O cliente vai receber uma nova cópia, montada com os dados atuais
              do pedido.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
